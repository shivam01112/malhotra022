!(function (e) {
  ...
  function f() {
    ...
    ((t = (function () {
      ...
      t &&
        jQuery
          .ajax({ url: e(l).attr("action"), data: a, type: "POST" })
          .done(function (t) {
            (p.removeClass("error"),
              p.addClass("success"),
              p.text(t),
  ...
  (e(l).on("submit", function (e) {
    ...
    v(".sidemenu-cart", ".sideMenuToggler", ".sideMenuCls", "show"),
    v(".sidemenu-info", ".sideMenuInfo", ".sideMenuCls", "show"),
    ...
    (e.fn.sectionPosition = function (t, a) {
      e(this).each(function () {
        ...
        ((s = Math.floor(c.height() / 2)),
          (n = c.attr(t)),
          (i = c.attr(a)),
          (o = g(e(i).css("padding-top"))),
          (r = g(e(i).css("padding-bottom"))),
          "top-half" === n
  ...
  (e(m).length &&
    e(m).imagesLoaded(function () {
      e(m).sectionPosition("data-sec-pos", "data-pos-for");
    }),
  ...
  var b = {
    ...
    lines: function () {
      return this.each(function () {
        ...
        y(e(this).children("br").replaceWith(t).end(), t, "line", "");
      });
    },
  };
  ((e.fn.lettering = function (t) {
    return t && b[t]
      ? b[t].apply(this, [].slice.call(arguments, 1))
      : "letters" !== t && t
  ...
  ((e.fn.inlineSvg = function () {
    ...
    e(".advance-search-btn").click(function (t) {
    ...
    e(document).click(function (t) {
      e(t.target).closest(
        ".advance-search-wrapper, .advance-search-btn, .select-group-wrapper",
      ).length ||
        (e(".advance-search-wrapper").removeClass("open").hide(),
        e(".advance-search-btn").removeClass("active"),
        e(".advance-search-btn i")
          .removeClass("fa-times")
          .addClass("fa-sliders-up"));
    }));
  const k = document.querySelector(".slider-drag-cursor"),
    x = { x: window.innerWidth / 2, y: window.innerHeight / 2 },
    T = { x: x.x, y: x.y },
    P = gsap.quickSetter(k, "x", "px"),
    D = gsap.quickSetter(k, "y", "px");
  (window.addEventListener("pointermove", (e) => {
    ...
    gsap.ticker.add(() => {
      ...
      ((x.x += (T.x - x.x) * e), (x.y += (T.y - x.y) * e), P(x.x), D(x.y));
    }),
    e(".cursor-hovered-area").hover(
      function () {
    ...
    e(".cursor-hovered-area .slider-pagination").hover(
      function () {
        e(".slider-drag-cursor").removeClass("active");
      },
      function () {
    ...
    (document.onkeydown = function (e) {
      return (
        ...
        void 0
            ...
            // Show modal on catalogue dropdown click
            document.querySelectorAll('.catalogue-option').forEach(function(el) {
                el.addEventListener('click', function(e) {
                    e.preventDefault();
                    // Set download link text and href based on option
                    let type = el.getAttribute('data-catalogue');
                    let link = '#';
                    let text = 'Download Catalogue';
                    if(type==='complete') { link = 'catalogue-complete.pdf'; text = 'Download Complete Catalogue'; }
                    if(type==='new') { link = 'catalogue-new-edition.pdf'; text = 'Download New Edition Catalogue'; }
                    if(type==='price') { link = 'catalogue-price-list.pdf'; text = 'Download Price List Catalogue'; }
                    document.getElementById('downloadLink').href = link;
                    document.getElementById('downloadLink').textContent = text;
                    // Reset forms
                    document.getElementById('catalogueForm').style.display = '';
                    document.getElementById('codeForm').style.display = 'none';
                    document.getElementById('downloadSection').style.display = 'none';
                    document.getElementById('catalogueForm').reset();
                    document.getElementById('codeForm').reset();
                    var modal = new bootstrap.Modal(document.getElementById('catalogueModal'));
                    modal.show();
                });
            });
            // On form submit, show code input
            document.getElementById('catalogueForm').addEventListener('submit', function(e) {
                e.preventDefault();
                document.getElementById('catalogueForm').style.display = 'none';
                document.getElementById('codeForm').style.display = '';
            });
            // On code submit, show download
